...
export function Uploader() {
    ...
    return (
        ...
        <form onSubmit={handleUploadSubmit}>
            <div className='my-6'>
                ...
                <div className="w-full my-4">
                    <label
                        ...
                        <input id='file' type='file' className="absolute left-0 top-0 w-full h-full opacity-0" webkitdirectory={allowDirectory} multiple={!allowDirectory} onChange={e => setFiles(Array.from(e.target.files))} required />
                    </label>
                </div>
            </div>
            ...
            {files.map((file, idx) => {
                return (
                    <div className={`rounded w-full flex justify-between p-4 ${idx % 2 === 0 ? 'bg-gray-900' : undefined}`}>
                        <span>{file.name}</span>
                        <span>{file.type}</span>
                        <span>{file.size}</span>
                    </div>
            ...
            <div className="flex justify-start gap-8 mt-6">
                <div className='mb3'>
                    <label>
                        <input className="bg-white" type='checkbox' value={allowDirectory} onChange={e => setAllowDirectory(e.target.checked)} /> Allow directory selection
                    </label>
                </div>
                {files.length === 1
                    ? (
                        <div className='mb3'>
                            <label>
                                <input className="bg-white" type='checkbox' value={wrapInDirectory} onChange={e => setWrapInDirectory(e.target.checked)} /> Wrap file in a directory
                            </label>
                        </div>
                    )
                    : null}
